...
--
    ...
    function m.platforms(wks)
        ...
        table.foreachi(platform_toolset_pairs, function(pair)
            ...
            for _, lib in pairs({ "kernel32.lib", "user32.lib", "gdi32.lib", "winspool.lib", "comdlg32.lib", "advapi32.lib", "shell32.lib", "ole32.lib", "oleaut32.lib", "uuid.lib", "odbc32.lib", "odbccp32.lib" }) do
...
---------------------------------------------------------------------------
    ...
    function m.solutionAllProject(wks)
        ...
        p.x(".ProjectBuildCommand = 'cd \"^$(SolutionDir)\" &amp; fbuild -config MoonEd.wks.bff -ide -monitor -dist -cache all-^$(Platform)-^$(Configuration)'")
        p.x(".ProjectRebuildCommand = 'cd \"^$(SolutionDir)\" &amp; fbuild -config MoonEd.wks.bff -ide -monitor -dist -cache -clean all-^$(Platform)-^$(Configuration)'")
        p.x(".ProjectCleanCommand = 'cd \"^$(SolutionDir)\" &amp; fbuild -config MoonEd.wks.bff -ide -monitor -dist -cache -clean'")
        p.pop("}")
    end
